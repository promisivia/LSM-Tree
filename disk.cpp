...
void DiskInfo::Compaction(size_t levelNow) {
	...
	if (levelNow == 0) {
		...
		filesToMerge = LevelList[levelNow]->pickFiles();//在该层选中间两个文件
		newLevel(levelNow + 1);
		if (!empty(levelNow + 1)) {
			//如果下一层存在文件，去下一层选取文件
			if (!SelectNextLevel(levelNow + 1, filesToMerge)) {
...
void DiskInfo::mergeFiles(std::vector<SSTable*> filesToMerge, size_t curLevel)
{
	...
	for (size_t i = 0; i < fileCount; i++)
	{
		...
		std::ifstream* infile = new std::ifstream("./dir/level"+ ToString(curLevel) +"/" + filesToMerge[i]->filename + ".txt", std::ios::binary);
